var portfolioInfo = {
    ...
    'hawkandheron': {
        ...
        'tech_description': "The website uses HTML and CSS to style the page and elements while using bootstrap to create responsive containers. I used Jekyll site generator as the framework for the project. Jekyll incorporates the Liquid render to easily loop through a list of image and render them to the page using the same format. I create reusables templates for the header and footer for each page. ",
    ...
    'beetsandeats': {
        ...
        'tech_description': "Beets and Eats uses data from GoogleMaps, Ticketmaster and Yelp to populate a map with restaurants and bars surrounding a desired event venue. We use AJAX calls and supply specific data for our request, such as a zip code for a location. Users enter a location and date in which we request information from Ticketmaster to populate a list of all the events occuring on the specified date. With the use of closures, when users click on the event, the website scrolls automatically to the map, which becomes populated with nearby restaurants and lounges from the Yelp API. Users can click on the icons to learn more about each location. ",
        'other_description': "The project was accomplished during a 2 day Hackathon. The initial preparation was creating quick wireframes and a SCRUM list of all the desired features. We created a repository on GitHub, and included a Javascript file incorporating JSDocs to specify the location and description for each function to be written. Once set up, we delegated tasks and a sprint using Meistertask. We Git as version control, and worked off of a development branch, with a separate branch for each feature. By using these task managing systems, we were able to produce a functioning and appealing web application!",
    ...
    'closeyourtabs': {
        ...
        'tech_description': "Close Your Tabs incorporates a React website, Node.js Javascript environment with mySQL database, and a Chrome extension. The three components join together to create a system to keep track of the current tabs in a Chrome browser session and access those tabs on a different browser or device. The extension uses Chrome Platform APIs to listen for events within the browser, such as creating a new tab, moving a tab, deleting a tab, etc. The extension popup displays the tabs and color codes them according to the time elapsed since last used. We used Google Oauth and passport for user authentication and cookie sessions. The extension is able to know if a user is logged in by injecting content script into the website to listen for the users actions with the website. This also enables injecting scripts to allow the user to manipulate their tabs from within the website as well. The extension communicates with the remote server using XHR requests.",
        'other_description': "The idea for Close Your Tabs spawned from observing developers with “mountains” of tabs, which titles can become difficult to read when searching for a particular website open. My contribution was developing the extension and writing the FAQ React component as well as UI/UX design for the project. We used Agile methodologies to create an increasingly more complex product. Meistertask was the centrally hub for delegating tasks, reporting bugs, and keeping track of excel sheets and API documentation we created. Git was used as version control.",
